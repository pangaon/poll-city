...
import { 
  ...
  FileText, Settings, Search, Bell, Plus, Zap, Shield,
  Crosshair, BookOpen, List, Navigation, User, Building2
} from "lucide-react";
...
const SIDEBAR_NAV = [
  { group: "War Room", items: [
    ...
    { name: "Voter Matrix", path: "/app/contacts", icon: Users },
    { name: "Signal Polling", path: "/app/polling", icon: BarChart2 },
  ]},
  { group: "Candidate", items: [
    { name: "Candidate Hub", path: "/app/candidate", icon: User },
    { name: "Elected Officials", path: "/app/officials", icon: Building2 },
  ]},
  { group: "Field Ops", items: [
    { name: "Field Command", path: "/app/field-ops", icon: Crosshair },
    { name: "Live Turf", path: "/app/canvassing", icon: MapIcon },
    { name: "Walk Lists", path: "/app/walk-list", icon: List },
    { name: "Lit Drops", path: "/app/lit-drops", icon: BookOpen },
    { name: "Ground Force", path: "/app/volunteers", icon: Heart },
    { name: "Deployments", path: "/app/signs", icon: MapPin },
  ]},
  { group: "Outreach", items: [
    { name: "Comms Grid", path: "/app/communications", icon: MessageSquare },
    { name: "Timeline", path: "/app/calendar", icon: Calendar },
    { name: "Action Items", path: "/app/tasks", icon: CheckSquare },
  ]},
  { group: "Resources", items: [
    { name: "Capital", path: "/app/donations", icon: DollarSign },
    { name: "Logistics", path: "/app/print", icon: Printer },
    { name: "Media Assets", path: "/app/media", icon: PlaySquare },
  ]},
  { group: "System", items: [
    { name: "Intelligence", path: "/app/reports", icon: FileText },
    { name: "Protocols", path: "/app/settings", icon: Settings },
    { name: "God Mode", path: "/app/admin", icon: Shield },
  ]}
];
...
export function AppLayout() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#050A1F] text-[#F5F7FF] font-sans overflow-hidden selection:bg-[#00E5FF]/30 selection:text-[#00E5FF]">
      ...
      {/* Global CSS for Glows and scrollbars */}
      <style>{`
      ...
      <aside className="w-[260px] flex-shrink-0 bg-[#0F1440]/90 backdrop-blur-xl border-r border-[#2979FF]/20 flex flex-col h-full relative z-20">
        <div className="h-16 flex items-center px-4 border-b border-[#2979FF]/20 bg-[#050A1F]/50">
          <Link to="/" className="flex items-center gap-3 text-[#00E5FF] font-black tracking-widest uppercase text-sm neon-text hover:text-[#F5F7FF] transition-colors">
                    ...
                    >
                      ...
                      <item.icon 
                        ...
                        className={cn(
                          ...
                          isActive ? "text-[#00E5FF] drop-shadow-[0_0_8px_rgba(0,229,255,0.8)]" : "text-[#6B72A0] group-hover:text-[#2979FF]"
                        )} 
                      />
                      {item.name}
                    </Link>
      ...
      <main className="flex-1 flex flex-col min-w-0 bg-[#050A1F] relative">
        ...
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] bg-[#2979FF]/10 rounded-full blur-[120px] pointer-events-none"></div>
        <div className="absolute bottom-[-20%] right-[-10%] w-[40%] h-[40%] bg-[#00E5FF]/5 rounded-full blur-[100px] pointer-events-none"></div>
        ...
        <header className="h-14 flex items-center justify-between px-6 border-b border-[#2979FF]/20 bg-[#0F1440]/70 backdrop-blur-md flex-shrink-0 z-10 relative">
          ...
          <div className="flex-1 flex items-center gap-6">
            ...
            <div className="relative w-80 group">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-[#2979FF] group-focus-within:text-[#00E5FF] transition-colors" size={16} />
              <input 
                type="text" 
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 px-3 py-1.5 rounded-md bg-[#FF3B30]/10 border border-[#FF3B30]/30 text-[#FF3B30] text-[11px] font-bold uppercase tracking-wider animate-pulse">
              <span className="w-2 h-2 rounded-full bg-[#FF3B30] shadow-[0_0_8px_#FF3B30]"></span>
              Live: 4 Alerts
            </div>
            
            <button className="relative text-[#6B72A0] hover:text-[#00E5FF] transition-colors p-2 rounded-lg hover:bg-[#2979FF]/10">
              <Bell size={18} />
              <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-[#FF3B30] shadow-[0_0_8px_#FF3B30]"></span>
            </button>
