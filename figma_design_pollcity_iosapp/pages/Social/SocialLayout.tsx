...
export function SocialLayout() {
  ...
  return (
    <div className="flex h-screen w-full bg-[#050A1F] items-center justify-center relative overflow-hidden font-sans">
      ...
      {/* App Switcher — floats above phone */}
      ...
      {/* ── Mobile Phone Frame ──────────────────────────────────── */}
      {/* Uses min() so it never exceeds viewport on any display    */}
      <div
      ...
      >
        ...
        <div className="h-12 w-full flex-shrink-0 absolute top-0 left-0 z-50 flex justify-between px-6 items-center pointer-events-none select-none">
          ...
          <div className="flex gap-1.5 items-center mt-1">
            ...
            <div className="flex items-end gap-[2px] h-3">
              {[3, 5, 7, 9].map((h, i) => (
                <div key={i} className="w-[3px] rounded-sm bg-white" style={{ height: `${h}px`, opacity: i < 3 ? 1 : 0.4 }} />
              ))}
            </div>
            {/* WiFi */}
        ...
        {/* Main Content Area — fills between status bar and nav */}
        <main className="flex-1 bg-[#0B0B0F] overflow-hidden relative z-0">
          <Outlet />
        </main>

        {/* Bottom Navigation */}
        <nav className="flex-shrink-0 bg-[#0B0B0F]/95 backdrop-blur-xl border-t border-[#1a1a22] flex items-center justify-around px-6 z-50 shadow-[0_-8px_30px_rgba(0,0,0,0.6)] relative"
          style={{ height: "min(82px, 11dvh)", paddingBottom: "clamp(4px, 2dvh, 20px)" }}>

          {/* Subtle top glow on nav */}
          ...
          {[
            { path: "/social", icon: Home, label: "Feed" },
            { path: "/social/trending", icon: Flame, label: "Trending" },
            { path: "/social/create", icon: PlusSquare, highlight: true, label: "" },
            { path: "/social/command", icon: Crosshair, label: "Field" },
            { path: "/social/profile", icon: UserIcon, label: "Profile" },
          ].map((item, i) => {
