...
function QRDisplay({ url, T }: { url: string; T: TC }) {
  // 9×9 QR cell grid stub — real impl uses qrcode.react
...
/* ─── VOTER NP OPTIONS (individual voter level when door answered) ─────── */
const VOTER_NP_OPTS = [
...
/* ─── PERSON CARD ─────────────────────────────────────────────────────── */
// 3-state machine: pending (unmarked+collapsed) → active (expanded) → done (marked+collapsed)
// Auto-advances via onMarkDone. Campaign fields only shown AFTER marking.
function PersonCard({ person, status, isActive, onActivate, onMarkDone,
  ...
  return (
    <motion.div layout className="rounded-2xl overflow-hidden"
      ...
      {/* Campaign fields — only shown after marking */}
      <AnimatePresence>
      ...
      {/* Done CTA — shown after any marking */}
      <AnimatePresence>
        {isMarked && (
      ...
      <div className={compact ? "py-0.5" : "py-1"}>
        ...
        <div className="flex flex-wrap gap-1">
          {field.options.map(opt => {
            const sel = v === opt;
            return (
              <button key={opt} onClick={() => onChange(sel ? undefined : opt)}
        ...
        {step === "pick" ? (
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl" style={{ backgroundColor: T.input, border: `1px solid ${T.border}` }}>
              ...
              <div className="flex items-center gap-3 ml-auto">
                {[-1, null, 1].map((d, i) => d === null
                  ...
                  : <button key={i} onClick={() => setQty(q => Math.max(1, Math.min(10, q + d)))}
          ...
          <motion.div key="c" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="space-y-3">
            <div className="p-4 rounded-xl" style={{ backgroundColor: `${cfg!.color}10`, border: `1.5px solid ${cfg!.color}40` }}>
              ...
              {note && <div className="text-[9px] italic mt-2" style={{ color: T.sub }}>"{note}"</div>}
            </div>
...
function LitDropMode({ stops, missionName, onCommitBatch, onCommitStop, onExit, T, isDark, teamData }: {
  ...
  const commit = (scope: "side" | "street" | "all") => {
    ...
    setCommitted(prev => new Set([...prev, ...ids]));
    ...
    <div className="h-full flex flex-col" style={{ backgroundColor: T.bg }}>
      <div className="px-5 pt-5 pb-3 flex-shrink-0">
        ...
        <div className="flex p-1 rounded-xl gap-1" style={{ backgroundColor: T.tabBg, border: `1px solid ${T.border}` }}>
          {tabBtns.map(b => (
            <button key={b.id} onClick={() => setTab(b.id as any)}
            ...
            <motion.div key="bd" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              <div className="rounded-xl p-4" style={{ backgroundColor: isDark ? "rgba(0,229,255,0.07)" : "rgba(25,118,210,0.05)", border: `1px solid ${T.borderB}` }}>
                ...
                <p className="text-[9px] leading-relaxed" style={{ color: T.sub }}>
                  Fields created here appear live in the canvass wizard Survey step and enrich all voter files. Lit pieces, info topics, and share channels also come from the backend — all configurable without a code release.
                </p>
              </div>
              ...
              <div className="rounded-xl p-4 space-y-3" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  ...
                  <select value={newFieldScope} onChange={e => setNewFieldScope(e.target.value as any)}
                    className="w-full px-2 py-1.5 rounded-lg text-[10px] focus:outline-none"
                    style={{ backgroundColor: T.input, border: `1px solid ${T.border}`, color: T.text }}>
            ...
            <motion.div key="pl" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-4">
              <div className="rounded-xl p-4" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
                ...
                <div className="flex items-center gap-2 px-3 py-2 rounded-lg" style={{ backgroundColor: isDark ? "rgba(255,255,255,0.04)" : T.deep }}>
                  ...
                  <span className="text-[9px]" style={{ color: T.sub }}>Shared Supabase backend · Role-based RLS · Separate bundles</span>
                </div>
              </div>
              <div className="rounded-xl p-4" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
                ...
                {[
                  ...
                  { title: "Option B — React Native / Expo", detail: "Port to React Native. Same Supabase data layer. True native performance.", color: "#2979FF" },
                  { title: "Option C — Swift / Kotlin Native", detail: "Full native rewrite using this prototype as functional spec and wireframe.", color: "#9C27B0" },
                ].map(o => (
...
export function SocialCommand() {
  ...
  // Which voter card is currently expanded — null = all collapsed (all-done state)
  ...
  const derivedOutcome = () => {
    ...
    // All voters NP-flagged → treat as not-home
    if (npFlags.length === household.length && household.length > 0) return "not-home";
    // "Hostile" NP flag counts as oppose
  ...
  const acceptMission = (m: typeof MISSIONS[0]) => {
    setActiveMission(m);
    setStops(ALL_STOPS.slice(m.start, m.end).map(s => ({ ...s, status: "pending" })));
  ...
  const StepHousehold = () => {
    ...
    // Active card: user-selected OR auto-first-unmarked
    const activeId = expandedPersonId ?? (unmarked[0]?.id ?? null);

    // First recorded outcome for "mark remaining as X" suggestion
    ...
    // All NP (no contact outcomes possible) — everyone flagged as not present
    const allNP = household.length > 0 && household.every(p => personStatuses[p.id]?.notPresent);

    // Bulk-mark all voters with one outcome
    const handleBulkMark = (outcomeId: string) => {
      household.forEach(p => {
        // Don't overwrite existing NP flags
        if (!personStatuses[p.id]?.notPresent) {
          setPersonContact(p.id, outcomeId);
        }
      });
      setExpandedPersonId(null);
    };

    // Mark remaining unmarked voters as the same as first
    ...
    // Called from PersonCard "Next Voter" / X button
    const handleMarkDone = (personId: number) => {
      const s = personStatuses[personId];
      const personIsMarked = s?.contact || s?.notPresent;
      if (personIsMarked) {
        // Advance to next unmarked (excluding current)
        const next = household.find(p =>
          p.id !== personId &&
          !personStatuses[p.id]?.contact &&
          !personStatuses[p.id]?.notPresent
        );
    ...
    // Handle contact set — auto-advance after setting
    ...
    // Handle NP set — auto-advance after setting
    ...
    return (
      <div className="space-y-3">
        ...
        {/* ── Quick bulk-mark strip (only before any votes, 2+ voters) ── */}
        ...
        {/* ── "Mark remaining as X" contextual strip ── */}
          ...
          {(allMarked || allNP) && (
            <motion.div initial={{ opacity: 0, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }}
              ...
              <div className="flex-1">
                ...
                <div className="text-[8px]" style={{ color: T.muted }}>
                  {allNP
                    ? "All voters flagged as not present — continuing to extras"
                    : "Tap any voter above to edit · continue to survey when ready"}
                </div>
              </div>
            </motion.div>
          )}
        ...
        {!allMarked && !allNP && markedCount > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            ...
            <span className="text-[8px]" style={{ color: "#FF9F0A" }}>
              {unmarked.length} voter{unmarked.length !== 1 ? "s" : ""} not yet recorded —{" "}
              tap their card to go back, or continue anyway.
            </span>
          </motion.div>
        )}

        {/* Single-voter no-action warning */}
        {household.length === 1 && markedCount === 0 && (
          <div className="text-[8px] text-center" style={{ color: T.muted }}>
            Record the voter's stance above, or use "Not at Door" if they weren't available.
          </div>
        )}
      </div>
    );
  ...
  const StepExtras = () => (
    <div className="space-y-4">
      ...
      <CandidateBell
        ...
        onSend={() => {
          setCandidateNotifSent(true);
        }}
        address={currentStop?.address ?? ""}
        T={T} isDark={isDark} />

      {/* Poll City Social invite */}
      <div className="rounded-xl p-3" style={{ backgroundColor: T.card, border: `1px solid ${T.border}` }}>
          ...
          {[
            ...
            return (
              <button key={ch.label} onClick={() => setInviteSent(true)}
                ...
                style={{ backgroundColor: done ? `${ch.color}12` : T.input, border: `1.5px solid ${done ? ch.color : T.border}`, color: done ? ch.color : T.muted }}>
                <I size={14} />
      ...
      <div className="rounded-xl p-3" style={{ backgroundColor: T.card, border: "1px solid rgba(255,59,48,0.25)" }}>
        ...
        <button onClick={() => { if (currentStop) { const ns = [...stops]; ns[stopIdx] = { ...currentStop, hasOpponentSign: !currentStop.hasOpponentSign }; setStops(ns); setOpponentCaptured(!opponentCaptured); } }}
  ...
  if (activeMission && litDropMode) return (
    <div className="h-full w-full" style={{ backgroundColor: T.bg }}>
      <LitDropMode stops={stops} missionName={activeMission.name} teamData={teamData}
        onCommitBatch={(mode, piece) => {
          const upd = stops.map(s => ({ ...s, status: "dropped" }));
          setStops(upd);
        }}
        onCommitStop={(id, piece) => {
          setStops(prev => prev.map(s => s.id === id ? { ...s, status: "dropped" } : s));
        }}
          ...
          {mainTab === "field" ? (
            <motion.div key="field" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 20 }} className="h-full overflow-y-auto px-4 pb-24 space-y-3 pt-2">
              <div className="text-[9px] font-black uppercase tracking-widest flex items-center gap-2 mb-2" style={{ color: T.sub }}>
                <Zap size={11} style={{ color: "#FFD600" }} /> Deployments · {ALL_STOPS.length} households
              </div>
