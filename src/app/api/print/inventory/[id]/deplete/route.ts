...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const parsed = depleteSchema.safeParse(rawBody);
  if (!parsed.success) {
  ...
  try {
    const updated = await prisma.$transaction(async (tx) => {
      ...
      // Deplete from reserved first, then from available
      const fromReserved = Math.min(qty, inv.reservedQty);
      const fromAvailable = qty - fromReserved;

      if (fromAvailable > inv.availableQty) throw new Error("INSUFFICIENT");

      const result = await tx.printInventory.update({
        where: { id: params.id },
        data: {
          reservedQty: { decrement: fromReserved },
          availableQty: { decrement: fromAvailable },
          depletedQty: { increment: qty },
        },
      });
      ...
      await tx.printInventoryLog.create({
        data: {
          ...
          notes: notes ?? `Depleted ${qty} units`,
