...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (campaignIds[0]) {
      await audit(prisma, 'volunteer.bulkDeactivate', {
    ...
    console.error("Bulk deactivate volunteers error:", error);
    return NextResponse.json({ error: "Failed to deactivate volunteers" }, { status: 500 });
  }
}