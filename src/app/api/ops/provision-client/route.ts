/**
 * /api/ops/provision-client
 *
 * Data Management console entrypoint for client provisioning.
 * Delegates to the existing /api/ops/provision logic by forwarding the request
 * internally, keeping all provisioning logic in one place.
 *
 * SUPER_ADMIN only.
 */
...
const ProvisionClientBody = z.object({
...
export async function POST(req: NextRequest) {
  ...
  const parsed = ProvisionClientBody.safeParse(raw);
  if (!parsed.success) {
  ...
  const { candidateName, campaignName, adminEmail, jurisdiction, plan } = parsed.data;
  ...
  // ── 4. Create membership (guard against duplicate) ──────────────────────────
  ...
  if (process.env.RESEND_API_KEY) {
    try {
          ...
          <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; color: #1f2937;">
            ...
            <div style="background: white; padding: 32px 24px; border: 1px solid #e5e7eb; border-top: none; border-radius: 0 0 12px 12px;">
              ...
              <p style="color: #9ca3af; font-size: 12px; margin: 16px 0 0;">
                Plan: ${plan.charAt(0).toUpperCase() + plan.slice(1)} · Poll City — Canadian Campaign Technology
              </p>
            </div>
          </div>
        `,
      ...
      console.error("[ops/provision-client] Email send failed:", emailErr);
    }
  }
  ...
  await audit(prisma, "client.provisioned", {
    ...
    details: { adminEmail, campaignName, isNewUser, emailSent, plan, source: "data-management-console" },
  });
