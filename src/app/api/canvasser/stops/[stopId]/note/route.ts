...
const bodySchema = z.object({ campaignId: z.string().min(1).optional(), note: z.string().min(1).max(5000) });
...
export async function POST(req: NextRequest, { params }: { params: { stopId: string } }) {
  ...
  try {
    const data = await doorService.addDoorNote({
      userId: ctx.userId,
    ...
    const message = e instanceof Error ? e.message : "Door note failed";
