...
const bodySchema = z.object({ campaignId: z.string().min(1).optional(), events: z.array(itemSchema).min(1).max(500) });
...
export async function POST(req: NextRequest) {
  ...
  const data = await litService.batchLogLitDrop(ctx.userId, ctx.campaignId, parsed.data.events);
