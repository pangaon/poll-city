...
const bodySchema = z.object({ campaignId: z.string().min(1).optional(), transcriptId: z.string().min(1) });
...
export async function POST(req: NextRequest) {
  ...
  try {
    const data = await adoniFieldService.parseFieldCommand(ctx.userId, ctx.campaignId, parsed.data.transcriptId);
    return NextResponse.json({ data });
  } catch (e) {
    const message = e instanceof Error ? e.message : "Adoni parse failed";
