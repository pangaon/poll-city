...
export async function POST(req: NextRequest) {
  ...
  if (!parsed.success) {
    return NextResponse.json({ error: "Validation failed", details: parsed.error.flatten().fieldErrors }, { status: 400, headers: NO_STORE_HEADERS });
  }
  ...
  if (!mimeMatch || !base64Part) {
    return NextResponse.json({ error: "Invalid image data" }, { status: 400, headers: NO_STORE_HEADERS });
  }

  const mimeType = mimeMatch[1];
  if (!mimeType.startsWith("image/")) {
    return NextResponse.json({ error: "Only image uploads are allowed" }, { status: 400, headers: NO_STORE_HEADERS });
  }

  const buffer = Buffer.from(base64Part, "base64");
  if (buffer.length > 12 * 1024 * 1024) {
    return NextResponse.json({ error: "Screenshot exceeds 12MB limit" }, { status: 413, headers: NO_STORE_HEADERS });
  }
