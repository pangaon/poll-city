...
function normalizeHeaderKey(key: string): string {
  ...
  const aliases: Record<string, string> = {
    ...
    poll: "municipalPoll",
    pollno: "municipalPoll",
    pollnumber: "municipalPoll",
    polldivision: "municipalPoll",
    pollingdivision: "municipalPoll",
    municipalpoll: "municipalPoll",
...
export async function POST(req: NextRequest) {
  ...
  const { forbidden: forbidden2 } = await guardCampaignRoute(session!.user.id, campaignId, "import_export:write");
