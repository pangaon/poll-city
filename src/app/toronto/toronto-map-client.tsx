...
const TORONTO_CENTER = { longitude: -79.3832, latitude: 43.6532, zoom: 10 };
...
export default function TorontoMapClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/atlas/toronto-addresses?${params.toString()}`)
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() as Promise<FeatureCollection>; })
  ...
  // suppress unused import warning — divider is used as a style reference below
  void divider;
    ...
    <div style={{ position: "relative", width: "100%", height: "100vh", overflow: "hidden", background: "#050e1c" }}>
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ color: "#fff", fontWeight: 800, fontSize: 15, letterSpacing: "0.05em" }}>CITY OF TORONTO</div>
      ...
      >
        ...
        <div style={{ padding: "10px 14px 2px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <div style={{ color: "rgba(255,255,255,0.28)", fontSize: 10, lineHeight: 1.6 }}>
            ...
            Source: Toronto Open Data (official)
          </div>
        </div>
      </motion.div>
        ...
        {selectedWard && selectedProps && !showTurfPanel && (
          ...
          >
            ...
            <div style={{ padding: "16px 18px" }}>
              ...
              <div style={{ marginTop: 10, color: "rgba(255,255,255,0.22)", fontSize: 10 }}>City of Toronto · Ontario Municipal 2026</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0" }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>
                {selectedAddress.source === "toronto-opendata" ? "Toronto Open Data" : "OpenStreetMap"}
              </span>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {wardLoading && (
          ...
          >
            ...
            <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading Toronto…</div>
            <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {wardError && (
          ...
          >
            ...
            <button onClick={() => { setWardError(null); setWardLoading(true); fetch("/api/atlas/toronto-wards").then(r => r.json() as Promise<FeatureCollection>).then(setWards).catch((e: Error) => setWardError(e.message)).finally(() => setWardLoading(false)); }}
