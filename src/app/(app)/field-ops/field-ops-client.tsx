...
// ── Sub-views (dynamically imported to keep initial bundle lean) ──────────────
...
type View = "assignments" | "map" | "walk" | "scripts" | "print";
...
const VIEW_TABS: { key: View; label: string; icon: React.ReactNode }[] = [
  ...
  { key: "print",       label: "Print Walk List", icon: <Printer className="h-4 w-4" /> },
];
...
export default function FieldOpsClient({ campaignId, campaignName, currentUserId, turfs, teamMembers }: Props) {
  ...
  return (
    <div className="space-y-0 p-4 sm:p-6">
      ...
      {view === "walk" && (
        <div className="pt-4">
          <WalkShell campaignId={campaignId} />
        </div>
      )}
                  ...
                  >
                    ...
                    {count !== undefined && (
                      <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${
                        isActive ? "bg-gray-100" : "bg-gray-100 text-gray-500"
                      }`}>
                        {count}
                      </span>
                    )}
                      ...
                      <tbody>
                        {assignments.map((a) => (
                          <motion.tr
                            ...
                            onClick={() => window.location.href = `/field-ops/${a.id}`}
                          >
                            ...
                            <td className="px-4 py-3" onClick={(e) => e.stopPropagation()}>
                              <div className="flex gap-1">
                                ...
                                {a.assignmentType === "canvass" && (
                                  <Button
                                    size="sm" variant="ghost" title="Print Walk List"
