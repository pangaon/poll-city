      ...
      {loading ? (
        ...
        <div key={o.id} className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
          <Link href={`/social/politicians/${o.id}`} className="flex items-start gap-3 group">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                <p className="font-bold text-gray-900 group-hover:text-blue-600 transition-colors">{o.name}</p>
          ...
          <div className="flex gap-2 mt-4">
            ...
            <Link href={`/social/politicians/${o.id}`} className="flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-xs font-semibold border-2 border-gray-200 text-gray-600 hover:border-blue-300 transition-all">
              <MessageSquare className="w-3.5 h-3.5" />View Profile
            </Link>
          </div>
        </div>
      ))}
