...
const MARKHAM_CENTER = { longitude: -79.2624, latitude: 43.8561, zoom: 11 };
...
export default function MarkhamMapClient() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/atlas/markham-addresses?${params.toString()}`)
      .then(r => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.json() as Promise<FeatureCollection>; })
  ...
  return (
    <div style={{ position: "relative", width: "100%", height: "100vh", overflow: "hidden", background: "#050e1c" }}>
      ...
      <MapGL
        ref={mapRef}
        initialViewState={MARKHAM_CENTER}
        mapStyle={TILE_STYLE}
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          ...
          <div>
            <div style={{ color: "#fff", fontWeight: 800, fontSize: 15, letterSpacing: "0.05em" }}>CITY OF MARKHAM</div>
      ...
      >
        <div style={{ padding: "0 14px 10px", borderBottom: "1px solid rgba(255,255,255,0.07)" }}>
          ...
          <div style={{ color: "#fff", fontSize: 14, fontWeight: 700, marginTop: 3 }}>Markham, Ontario</div>
        </div>
        ...
        <div style={{ padding: "10px 14px 2px", borderTop: "1px solid rgba(255,255,255,0.07)" }}>
          <div style={{ color: "rgba(255,255,255,0.28)", fontSize: 10, lineHeight: 1.6 }}>
            ...
            Source: Markham Open Data (official)
          </div>
        </div>
      </motion.div>
        ...
        {selectedWard && selectedProps && !showTurfPanel && (
          ...
          >
            ...
            <div style={{ padding: "16px 18px" }}>
              ...
              <div style={{ marginTop: 10, color: "rgba(255,255,255,0.22)", fontSize: 10 }}>York Region · Ontario Municipal 2026</div>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
          ...
          >
            ...
            <div style={{ display: "flex", justifyContent: "space-between", padding: "4px 0" }}>
              ...
              <span style={{ color: "rgba(255,255,255,0.4)", fontSize: 11 }}>
                {selectedAddress.source === "markham-geohub" ? "Markham Open Data" : "OpenStreetMap"}
              </span>
            </div>
          </motion.div>
        )}
      ...
      <AnimatePresence>
        {wardLoading && (
          ...
          >
            ...
            <div style={{ color: "rgba(255,255,255,0.55)", fontSize: 13, letterSpacing: "0.1em", textTransform: "uppercase" }}>Loading Markham…</div>
            <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <AnimatePresence>
        {wardError && (
          ...
          >
            ...
            <button onClick={() => { setWardError(null); setWardLoading(true); fetch("/api/atlas/markham-wards").then(r => r.json() as Promise<FeatureCollection>).then(setWards).catch((e: Error) => setWardError(e.message)).finally(() => setWardLoading(false)); }}
