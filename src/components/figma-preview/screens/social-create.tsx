...
import { X, Zap, Sparkles, ChevronDown } from "lucide-react";
  ...
  return (
    <div className="h-full w-full bg-[#0B0B0F] overflow-y-auto pt-14 pb-6">
      ...
      <div className="px-6 space-y-6">
                ...
                {selectedCategories.length > 0 && (
                  <div className="flex flex-wrap gap-2 mb-6">
                    {selectedCategories.map((cat, i) => (
                      <span key={i} className="text-[10px] font-black uppercase tracking-[0.2em] bg-[#FFFFFF]/10 backdrop-blur-xl px-3 py-1.5 rounded border border-[#FFFFFF]/20">{cat}</span>
                    ))}
                  </div>
                )}
                <h2 className="text-4xl font-black leading-[1.1] tracking-tighter drop-shadow-[0_5px_20px_rgba(0,0,0,1)] min-h-[88px]">
        ...
        {/* Question Input */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-[#AAB2FF] mb-3 block">Poll Question</label>
          <textarea
            value={question}
          ...
          <div className="flex justify-between items-center mt-2 text-[10px] font-bold uppercase tracking-widest">
            ...
            <span className={cn(question.length >= 10 ? "text-[#00E676]" : "text-[#6C6F7F]")}>{question.length} / 180</span>
          </div>
        </div>

        {/* Categories */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-[#AAB2FF] mb-3 block">Categories (Select 1-3)</label>
          <button
        ...
        {/* Background Theme */}
        <div>
          <label className="text-[10px] font-black uppercase tracking-widest text-[#AAB2FF] mb-3 block">Visual Theme</label>
