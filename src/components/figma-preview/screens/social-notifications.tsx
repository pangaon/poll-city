...
export function SocialNotifications() {
  return (
    <div className="h-full w-full bg-[#050A1F] overflow-y-auto pt-14 pb-24 relative">
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[10%] right-[-10%] w-[200px] h-[200px] bg-[#2979FF]/10 rounded-full blur-[80px]" />
        <div className="absolute bottom-[30%] left-[-10%] w-[150px] h-[150px] bg-[#00E5FF]/10 rounded-full blur-[60px]" />
      </div>
      ...
      <header className="px-6 mb-6 sticky top-14 bg-[#050A1F]/90 backdrop-blur-xl z-20 pb-4 pt-2 border-b border-[#2979FF]/20 shadow-[0_10px_30px_rgba(0,0,0,0.5)]">
        <div className="flex items-center gap-2">
          <div className="relative">
            ...
            <div className="absolute inset-0 bg-[#00E5FF] blur-md opacity-50 animate-pulse" />
          </div>
        ...
        {NOTIFICATIONS.map((n, i) => (
          ...
          >
            <div
              ...
              style={{ backgroundColor: `${n.color}15`, borderColor: `${n.color}40`, boxShadow: `0 0 15px ${n.color}30` }}
            >
              <n.icon size={18} style={{ color: n.color, filter: `drop-shadow(0 0 8px ${n.color})` }} />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex justify-between items-start mb-1">
                <h3 className="text-[13px] font-bold text-[#F5F7FF] tracking-tight truncate group-hover:text-[#00E5FF] transition-colors">{n.title}</h3>
                <span className="text-[9px] font-mono text-[#6B72A0] whitespace-nowrap ml-2 mt-0.5">{n.time}</span>
              </div>
              <p className="text-[11px] text-[#AAB2FF] leading-relaxed line-clamp-2">{n.desc}</p>
            </div>
          </motion.div>
        ))}
      </div>
