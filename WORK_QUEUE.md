...
| AtlasMapClient Phase 3 — turf cutting overhaul: street search, manual street-to-turf assignment, volunteer DB assignment, two-mode turf builder | DONE — commit 7fe5cf1 | atlas-map-client.tsx: ⚡ Quick / ✏️ Manual mode tabs, street search + checkbox list, AnimatePresence confirm dialog, volunteer combobox pulling from /api/atlas/volunteers-for-map (auth-gated, campaignId-scoped). New: src/app/api/atlas/volunteers-for-map/route.ts. No schema changes. George browser confirmation pending. |
