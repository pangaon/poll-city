...
| Hardened Ontario Ward Infrastructure — 3-layer DB cache (WardBoundary table), universal ingestor, 28-municipality registry, daily cron, ETag headers | DONE — commits baef811 + dc0b180 | WardBoundary Prisma model, src/config/ward-asset-registry.ts (28 municipalities), src/lib/atlas/ward-ingestor.ts, seed endpoint /api/atlas/seed-wards, cron /api/cron/refresh-wards (3am daily in vercel.json), brampton-addresses fixed (maps1.brampton.ca). Build fixed dc0b180 (wrong prisma import path + type casts). George must: (1) npx prisma db push to create ward_boundaries table, (2) hit /api/atlas/seed-wards?secret=CRON_SECRET once to populate. See GEORGE_TODO item 3 + 3f. |
| Ontario Election Results Overlay — Whitby test run: 6 Ontario Open Data CSVs (2014/2018/2022), seed script, GET /api/atlas/election-results, toggle layer + panel in atlas-map-client, wired to whitby config | DONE — commit pending push | data/ontario-elections/ (6 CSVs), scripts/seed-ontario-elections.ts, src/app/api/atlas/election-results/route.ts. Election toggle in whitby header — turnout choropleth + race results panel per year. No schema migration needed (uses existing ElectionResult table). George must run: npx tsx scripts/seed-ontario-elections.ts --municipality "Whitby T" to populate DB. |
